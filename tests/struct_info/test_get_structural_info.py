...
def test_secondary_structure_helix_matches_pyrosetta():
   ...
   with h5py.File(reference_path) as ref:
      ...
      for protein in res_ids:
         for item in protein:
            pdb = b"1hmd" if item[1] == b"1hm" else item[1] # annoying bug in reference implementation with .strip("pdb")
...
def test_number_of_atoms_matches_pyrosetta():
   with h5py.File(reference_path) as ref:
      ref_atoms = ((ref["data"]["elements"] != b"H") & (ref["data"]["elements"] != b"")).sum()
      ref_H = (ref["data"]["elements"] == b"H").sum()

   with h5py.File(test_path) as test:
      test_atoms = ((test["data"]["elements"] != b"H") & (test["data"]["elements"] != b"")).sum()
